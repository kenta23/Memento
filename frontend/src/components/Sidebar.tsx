import { Plus, ScrollText, ArchiveRestore, Heart, Tags, Folders } from 'lucide-react'
...
export default function Sidebar() {
  ...
  return (
    <div className="min-w-[170px] w-[270px] lg:w-[290px] border border-[#3DA6E1] h-auto min-h-screen">
      <div className="w-full">
          
          <div className='mx-auto mt-4 flex items-center w-full justify-center'>
             <Link to={'/create-new'}  className={`${location.pathname === '/create-new' ? 'bg-[#79878f]' : 'bg-[#1698E1]'} px-4 flex items-center gap-2  py-2 text-white `}>
          ...
          <div className='mt-[40px] px-3 py-2 w-full border-b border-b-[#3DA6E1] flex flex-col gap-2 items-start'>
               ...
               <div className='flex mt-2 px-3 rounded-xl flex-col gap-3 h-auto w-auto'>   
                   {noteOptions.map(item => (
                      <div key={item.id} className={`${location.pathname === item.path ? 'bg-[#3B95C8]' : '' } min-w-[200px] hover:bg-[#3B95C8]  duration-150 ease-out group  p-2 rounded-xl min-h-[40px]`}>
                       <NavLink to={item.path}  className={`flex group-hover:text-white ${location.pathname === item.path ? 'text-white' : 'text-[#3B95C8]'} items-center w-full h-full  gap-2 flex-row` }>
                          {item.icon}
               ...
               <div className='flex mt-2 px-3 rounded-xl flex-col gap-3 h-auto w-auto'>   
                   {tagsOptions.map(item => (
                      <div key={item.id} className='min-w-[200px] hover:bg-[#3B95C8] duration-150 ease-out group p-2 rounded-xl min-h-[40px] '>
                       <NavLink to="/all-notes" className={`flex group-hover:text-white  items-center w-full h-full text-[#3B95C8] gap-2 flex-row` }>
                          <Tags size={24} />
